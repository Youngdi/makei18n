...
exports.makeCSVFile = R.curryN(3 , (outputFileName, outputDir ,data) => {
    const exists = fs.existsSync(outputDir);
    if (exists) {
      fs.writeFileSync(`${outputDir}/${outputFileName}`, data, {encoding: 'utf8'});
    } else {
      rimraf.sync(outputDir);
      fs.mkdirSync(outputDir.replace(/\./g, '').replace(/\//g,''));
      fs.writeFileSync(`${outputDir}/${outputFileName}`, data, {encoding: 'utf8'});
    }
  }
);

exports.generateDir = async (langList, i18nLanguageTransfer, outputDir) =>  {
  const exists = fs.existsSync(outputDir);
  if (exists) {
    langList.forEach((item, i) => {
      const i18nExists = fs.existsSync(`${outputDir}/${i18nLanguageTransfer(item)}`);
      !i18nExists && fs.mkdirSync(`${outputDir}/${i18nLanguageTransfer(item)}`);
    });
  } else {
    rimraf.sync(outputDir);
    fs.mkdirSync(outputDir.replace(/\./g, '').replace(/\//g,''));
    langList.forEach((item, i) => {
      fs.mkdirSync(`${outputDir}/${i18nLanguageTransfer(item)}`);
    });
  }
};
...
exports.webGetCSVLangList = (csvString) => new Promise((resolve, reject) => {
  csv().fromString(csvString).on('end_parsed', (jsonObj) =>resolve(jsonObj));
});
...
exports.makeJSONfile = (dirLangList, i18nLanguageTransfer, env, outputFileName, outputDir) => async (mappingLangList) => {
  mappingLangList.forEach((item, i) => {
    const outFilePath = (env == 'ChromeExtension') ? `${outputDir}/${i18nLanguageTransfer(dirLangList[i])}/messages.json` : `${outputDir}/${i18nLanguageTransfer(dirLangList[i])}/${outputFileName}`;
    fs.writeFileSync(outFilePath, JSON.stringify(item), {encoding: 'utf8'})
  });
};

exports.webMakeJSONfileZip = (dirLangList, i18nLanguageTransfer, env, outputFileName) => async (mappingLangList) => {
  mappingLangList.forEach((item, i) => {
    zip.folder("_locales").folder(i18nLanguageTransfer(dirLangList[i])).file(outputFileName, JSON.stringify(item));
  });
...
exports.makeLangJSON = R.curryN(3, (envPreloadFN, val, langString) => R.pipe(
  R.converge(R.zipObj, 
    [
      R.pluck('key'),
      R.pipe(
...
/**
 * migrateJSONWithConfig :: [Object] csvLangList, String val  => (csvLangList, val, i18nLanguageTransfer) -> Object
...
exports.webGenerateLangList = (dirLangList, i18nLanguageTransfer, makeLangJSONWithEnv) => async (csvLangList) => dirLangList.reduce((acc, val) => [...acc, makeLangJSONWithEnv(val, csvLangList)], []);
...
exports.webGetDirLangList = (csvString) => new Promise((resolve, reject) => {
  csv().fromString(csvString).on('end_parsed', (jsonObj) => resolve(R.without(['key'], R.keys(jsonObj[0]))));
});
